...
print("=" * 60)
df = pd.read_csv('/mnt/user-data/uploads/sample_04.csv')
...
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 2: Data Leakage Audit")
print("=" * 60)

leakage_report = []

TARGET = 'Membership_Renewal_Decision'

# --- LEAKAGE CHECK 1: Post-outcome columns ---
# Desire_To_Cancel and Renewal_Impact_Due_to_Price_Increase are
# ONLY populated when target is known → they are outcomes, not predictors.
for col in ['Desire_To_Cancel', 'Renewal_Impact_Due_to_Price_Increase']:
    overlap = df[df[TARGET].notna() & df[col].notna()].shape[0]
    null_when_target_null = df[df[TARGET].isna() & df[col].isna()].shape[0]
    leakage_report.append({
        'Column': col,
        'Leakage_Type': 'Post-outcome / Target-derived',
        'Evidence': f'Always NULL when target is NULL ({null_when_target_null} rows). Only populated alongside target.',
        'Action': 'DROP — causes data leakage'
    })
    print(f"  ⚠️  LEAKAGE: {col} → DROP (post-outcome, only exists when target is known)")

# --- LEAKAGE CHECK 2: Prospect_Outcome ---
# Prospect_Outcome=Churned strongly maps to target=Yes (cancelled)
# Prospect_Outcome=Won maps to target=No (renewed)
# This is a business outcome label — direct proxy of the target
ct = pd.crosstab(df['Prospect_Outcome'].fillna('NaN'),
                 df[TARGET].fillna('NaN'))
leakage_report.append({
    'Column': 'Prospect_Outcome',
    'Leakage_Type': 'Direct target proxy',
    'Evidence': f"Churned rows: {ct.loc['Churned','Yes'] if 'Churned' in ct.index else 0} Yes / {ct.loc['Churned','No'] if 'Churned' in ct.index else 0} No. Won rows: {ct.loc['Won','Yes'] if 'Won' in ct.index else 0} Yes / {ct.loc['Won','No'] if 'Won' in ct.index else 0} No.",
    'Action': 'DROP — direct business outcome label, same information as target'
})
print(f"  ⚠️  LEAKAGE: Prospect_Outcome → DROP (Churned=cancelled=Yes, Won=renewed=No)")

# --- LEAKAGE CHECK 3: Highly correlated numerical columns ---
num_cols = ['Total_Renewal_Score_New', 'Status_Scores', 'Sustainability_Score']
...
if high_corr_pairs:
    for a, b, c in high_corr_pairs:
        leakage_report.append({
            'Column': f'{a} ↔ {b}',
            'Leakage_Type': 'Multicollinearity (not leakage per se)',
            'Evidence': f'Pearson r = {c}',
            'Action': f'KEEP both but note high correlation — consider dropping {b} if overfitting'
        })
        print(f"  ⚠️  HIGH CORRELATION: {a} ↔ {b} (r={c}) → keep but monitor")

# --- LEAKAGE CHECK 4: cc_*_x / cc_*_y duplicates ---
# _x and _y are clearly the same feature collected at two time points
# Nearly all values are NULL in the training rows
leakage_report.append({
    'Column': 'cc_*_x and cc_*_y columns (6 total)',
    'Leakage_Type': 'Near-empty in training rows',
    'Evidence': '~95% NULL in rows where target is known (19026/20123 rows missing)',
    'Action': 'DROP — too sparse to be useful; risk of noise'
})
print(f"  ⚠️  NEAR-EMPTY: cc_*_x / cc_*_y columns → DROP (>94% missing in labelled rows)")

# Save leakage report
leakage_df = pd.DataFrame(leakage_report)
leakage_df.to_csv(f"{OUTPUT_DIR}/leakage_audit_report.csv", index=False)
print(f"\n  ✅  Leakage audit saved → leakage_audit_report.csv")
